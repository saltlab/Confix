...
function dg(x) {
  return confixWrapper("return", "return document.getElementById(x);", [""], [], "dg", document.getElementById(x));
}
...
function toggleInfo(wut) {
  if (confixWrapper("condition", "dg('hin').innerHTML", [""], [], "toggleInfo", confixWrapper("functionCall", "dg('hin')", ["hin"], ['hin'], "toggleInfo", dg('hin')).innerHTML)) 
  if (confixWrapper("condition", "(!wut) || (wut == '')", [""], [], "toggleInfo", (!wut) || (wut == ''))) 
  wut = confixWrapper("infix", "wut = dg('hin').innerHTML", [""], [], "toggleInfo", confixWrapper("functionCall", "dg('hin')", ["hin"], ['hin'], "toggleInfo", dg('hin')).innerHTML);
  if (confixWrapper("condition", "wut == 'Show'", [""], [], "toggleInfo", wut == 'Show')) 
  {
    confixWrapper("functionCall", "dg('hin')", ["hin"], ['hin'], "toggleInfo", dg('hin')).innerHTML = confixWrapper("infix", "dg('hin').innerHTML = 'Hide&nbsp;'", [""], [], "toggleInfo", 'Hide&nbsp;');
    confixWrapper("functionCall", "dg('photoBoxes')", ["photoBoxes"], ['photoBoxes'], "toggleInfo", dg('photoBoxes')).style.display = confixWrapper("infix", "dg('photoBoxes').style.display = 'block'", [""], [], "toggleInfo", 'block');
    confixWrapper("functionCall", "dg('theImage')", ["theImage"], ['theImage'], "toggleInfo", dg('theImage')).style.cssFloat = confixWrapper("infix", "dg('theImage').style.cssFloat = 'left'", [""], [], "toggleInfo", 'left');
    confixWrapper("functionCall", "dg('theImage')", ["theImage"], ['theImage'], "toggleInfo", dg('theImage')).style.styleFloat = confixWrapper("infix", "dg('theImage').style.styleFloat = 'left'", [""], [], "toggleInfo", 'left');
    confixWrapper("functionCall", "dg('theImage')", ["theImage"], ['theImage'], "toggleInfo", dg('theImage')).style.marginRight = confixWrapper("infix", "dg('theImage').style.marginRight = '15px'", [""], [], "toggleInfo", '15px');
    confixWrapper("functionCall", "setCookie('hideinfo', 'false')", ["hideinfo", "false"], ['hideinfo', 'false'], "toggleInfo", setCookie('hideinfo', 'false'));
  } else {
    confixWrapper("functionCall", "dg('hin')", ["hin"], ['hin'], "toggleInfo", dg('hin')).innerHTML = confixWrapper("infix", "dg('hin').innerHTML = 'Show'", [""], [], "toggleInfo", 'Show');
    confixWrapper("functionCall", "dg('photoBoxes')", ["photoBoxes"], ['photoBoxes'], "toggleInfo", dg('photoBoxes')).style.display = confixWrapper("infix", "dg('photoBoxes').style.display = 'none'", [""], [], "toggleInfo", 'none');
    confixWrapper("functionCall", "dg('theImage')", ["theImage"], ['theImage'], "toggleInfo", dg('theImage')).style.cssFloat = confixWrapper("infix", "dg('theImage').style.cssFloat = 'none'", [""], [], "toggleInfo", 'none');
    confixWrapper("functionCall", "dg('theImage')", ["theImage"], ['theImage'], "toggleInfo", dg('theImage')).style.styleFloat = confixWrapper("infix", "dg('theImage').style.styleFloat = 'none'", [""], [], "toggleInfo", 'none');
    confixWrapper("functionCall", "dg('theImage')", ["theImage"], ['theImage'], "toggleInfo", dg('theImage')).style.marginRight = confixWrapper("infix", "dg('theImage').style.marginRight = '55px'", [""], [], "toggleInfo", '55px');
...
function cookieVal(cookname) {
  ...
  for (i = confixWrapper("infix", "i = 0", [""], [], "cookieVal", 0); i < thiscook.length; i++) 
    ...
    return confixWrapper("return", "return thiscook[i].split(\"=\")[1];", [""], [], "cookieVal", confixWrapper("functionCall", "thiscook[i].split(\"=\")", ["="], ["="], "cookieVal", thiscook[i].split("="))[1]);
...
function rand(x) {
  return confixWrapper("return", "return Math.round(Math.random() * x);", [""], [], "rand", Math.round(confixWrapper("functionCall", "Math.random()", [], [], "rand", Math.random()) * x));
}
function reshuffle() {
  var maxRand = confixWrapper("initvar", "var maxRand = 400 - 75", [""], [], "reshuffle", 400 - 75);
  var n = confixWrapper("initvar", "var n = dg('thumbscount').value", [""], [], "reshuffle", confixWrapper("functionCall", "dg('thumbscount')", ["thumbscount"], ['thumbscount'], "reshuffle", dg('thumbscount')).value);
  for (var i = confixWrapper("initvar", "var i = 0", [""], [], "reshuffle", 0); i < n; i++) 
    {
      confixWrapper("functionCall", "dg('ThumbInBox' + i)", ["ThumbInBox + i"], ['ThumbInBox' + i], "reshuffle", dg('ThumbInBox' + i)).style.top = confixWrapper("infix", "dg('ThumbInBox' + i).style.top = rand(maxRand) + 'px'", [""], [], "reshuffle", confixWrapper("functionCall", "rand(maxRand)", ["maxRand"], [maxRand], "reshuffle", rand(maxRand)) + 'px');
      confixWrapper("functionCall", "dg('ThumbInBox' + i)", ["ThumbInBox + i"], ['ThumbInBox' + i], "reshuffle", dg('ThumbInBox' + i)).style.left = confixWrapper("infix", "dg('ThumbInBox' + i).style.left = rand(maxRand) + 'px'", [""], [], "reshuffle", confixWrapper("functionCall", "rand(maxRand)", ["maxRand"], [maxRand], "reshuffle", rand(maxRand)) + 'px');
    }
}
function updateIndic() {
  var v = confixWrapper("initvar", "var v = dg('indicator').innerHTML", [""], [], "updateIndic", confixWrapper("functionCall", "dg('indicator')", ["indicator"], ['indicator'], "updateIndic", dg('indicator')).innerHTML);
  ...
  confixWrapper("functionCall", "dg('indicator')", ["indicator"], ['indicator'], "updateIndic", dg('indicator')).innerHTML = confixWrapper("infix", "dg('indicator').innerHTML = '&#149;      ' + v", [""], [], "updateIndic", '&#149;      ' + v); else confixWrapper("functionCall", "dg('indicator')", ["indicator"], ['indicator'], "updateIndic", dg('indicator')).innerHTML = confixWrapper("infix", "dg('indicator').innerHTML = '&nbsp; ' + v", [""], [], "updateIndic", '&nbsp; ' + v);
  if (confixWrapper("condition", "isAjaxing", [""], [], "updateIndic", isAjaxing)) 
  confixWrapper("functionCall", "setTimeout(\"updateIndic();\", 500)", ["updateIndic();", "500"], ["updateIndic();", 500], "updateIndic", setTimeout("updateIndic();", 500)); else confixWrapper("functionCall", "dg('indicator')", ["indicator"], ['indicator'], "updateIndic", dg('indicator')).innerHTML = confixWrapper("infix", "dg('indicator').innerHTML = ''", [""], [], "updateIndic", '');
}
function alertContents(http_request) {
  ...
  try {
    if (confixWrapper("condition", "http_request.status == 200", [""], [], "alertContents", http_request.status == 200)) 
    {
      ...
      {
        confixWrapper("functionCall", "dg('rateStatus')", ["rateStatus"], ['rateStatus'], "alertContents", dg('rateStatus')).innerHTML = confixWrapper("infix", "dg('rateStatus').innerHTML = 'Your rating saved!'", [""], [], "alertContents", 'Your rating saved!');
        confixWrapper("functionCall", "dg('sumRate')", ["sumRate"], ['sumRate'], "alertContents", dg('sumRate')).innerHTML = confixWrapper("infix", "dg('sumRate').innerHTML = AjaxVal.substr(4, AjaxVal.length - 1)", [""], [], "alertContents", AjaxVal.substr(4, AjaxVal.length - 1));
      }
      ...
      {
        confixWrapper("functionCall", "dg('wvwimg')", ["wvwimg"], ['wvwimg'], "alertContents", dg('wvwimg')).src = confixWrapper("infix", "dg('wvwimg').src = \"wv.php?rand=\" + rand(10000000)", [""], [], "alertContents", "wv.php?rand=" + confixWrapper("functionCall", "rand(10000000)", ["10000000"], [10000000], "alertContents", rand(10000000)));
...
function confirmDelete(x) {
  return confixWrapper("return", "return confirm('Are you sure you want to delete \"' + x + '\"?');", [""], [], "confirmDelete", confirm('Are you sure you want to delete "' + x + '"?'));
}
function hideElem(x) {
  try {
    confixWrapper("functionCall", "dg(x)", ["x"], [x], "hideElem", dg(x)).style.display = confixWrapper("infix", "dg(x).style.display = 'none'", [""], [], "hideElem", 'none');
  }  catch (e) {
}
}
function showElem(x) {
  try {
    confixWrapper("functionCall", "dg(x)", ["x"], [x], "showElem", dg(x)).style.display = confixWrapper("infix", "dg(x).style.display = 'block'", [""], [], "showElem", 'block');
  }  catch (e) {
}
}
function inlineElem(x) {
  try {
    confixWrapper("functionCall", "dg(x)", ["x"], [x], "inlineElem", dg(x)).style.display = confixWrapper("infix", "dg(x).style.display = 'inline'", [""], [], "inlineElem", 'inline');
  }  catch (e) {
}
}
function tableRowElem(x) {
  try {
    ...
    confixWrapper("functionCall", "dg(x)", ["x"], [x], "tableRowElem", dg(x)).style.display = confixWrapper("infix", "dg(x).style.display = 'table-row'", [""], [], "tableRowElem", 'table-row');
  }  catch (e) {
  if (confixWrapper("condition", "dg(x)", [""], [], "tableRowElem", dg(x))) 
  confixWrapper("functionCall", "dg(x)", ["x"], [x], "tableRowElem", dg(x)).style.display = confixWrapper("infix", "dg(x).style.display = 'inline'", [""], [], "tableRowElem", 'inline');
}
}
function checkWV() {
  re = confixWrapper("infix", "re = /^d{5}$/", [""], [], "checkWV", /^\d{5}$/);
  if (confixWrapper("condition", "!re.test(dg('wvinput').value)", [""], [], "checkWV", !confixWrapper("functionCall", "re.test(dg('wvinput').value)", ["dg(wvinput).value"], [confixWrapper("functionCall", "dg('wvinput')", ["wvinput"], ['wvinput'], "checkWV", dg('wvinput')).value], "checkWV", re.test(confixWrapper("functionCall", "dg('wvinput')", ["wvinput"], ['wvinput'], "checkWV", dg('wvinput')).value)))) 
  {
  ...
  if (confixWrapper("condition", "dg('cmntTextArea').value.length == 0", [""], [], "checkWV", confixWrapper("functionCall", "dg('cmntTextArea')", ["cmntTextArea"], ['cmntTextArea'], "checkWV", dg('cmntTextArea')).value.length == 0)) 
  {
...
function doReply(x) {
  ...
  confixWrapper("functionCall", "dg('cmntReply')", ["cmntReply"], ['cmntReply'], "doReply", dg('cmntReply')).value = confixWrapper("infix", "dg('cmntReply').value = x", [""], [], "doReply", x);
...
function toggle(w, c, t) {
  var block = confixWrapper("initvar", "var block = (dg(w).style.display == 'block')", [""], [], "toggle", (confixWrapper("functionCall", "dg(w)", ["w"], [w], "toggle", dg(w)).style.display == 'block'));
  confixWrapper("functionCall", "dg(w)", ["w"], [w], "toggle", dg(w)).style.display = confixWrapper("infix", "dg(w).style.display = (block) ? 'none' : 'block'", [""], [], "toggle", (block) ? 'none' : 'block');
  confixWrapper("functionCall", "dg(c)", ["c"], [c], "toggle", dg(c)).style.display = confixWrapper("infix", "dg(c).style.display = (!block) ? 'none' : 'block'", [""], [], "toggle", (!block) ? 'none' : 'block');
...
function ss_update() {
  ...
  if (confixWrapper("condition", "ss_cur >= ss_date.length", [""], [], "ss_update", ss_cur >= ss_date.length)) 
  {
    ...
    confixWrapper("functionCall", "dg('ss_n')", ["ss_n"], ['ss_n'], "ss_update", dg('ss_n')).innerHTML = confixWrapper("infix", "dg('ss_n').innerHTML = \"Final\"", [""], [], "ss_update", "Final");
    ...
    ss_loaded = confixWrapper("infix", "ss_loaded = (dg('ss_photo').src == ss_src[ss_cur])", [""], [], "ss_update", (confixWrapper("functionCall", "dg('ss_photo')", ["ss_photo"], ['ss_photo'], "ss_update", dg('ss_photo')).src == ss_src[ss_cur]));
    ...
    confixWrapper("functionCall", "dg('ss_date')", ["ss_date"], ['ss_date'], "ss_update", dg('ss_date')).innerHTML = confixWrapper("infix", "dg('ss_date').innerHTML = ss_date[ss_cur]", [""], [], "ss_update", ss_date[ss_cur]);
    confixWrapper("functionCall", "dg('ss_title')", ["ss_title"], ['ss_title'], "ss_update", dg('ss_title')).innerHTML = confixWrapper("infix", "dg('ss_title').innerHTML = ss_ttl[ss_cur]", [""], [], "ss_update", ss_ttl[ss_cur]);
    confixWrapper("functionCall", "dg('ss_desc')", ["ss_desc"], ['ss_desc'], "ss_update", dg('ss_desc')).innerHTML = confixWrapper("infix", "dg('ss_desc').innerHTML = ss_desc[ss_cur]", [""], [], "ss_update", ss_desc[ss_cur]);
    confixWrapper("functionCall", "dg('ss_n')", ["ss_n"], ['ss_n'], "ss_update", dg('ss_n')).innerHTML = confixWrapper("infix", "dg('ss_n').innerHTML = 1 + ss_cur", [""], [], "ss_update", 1 + ss_cur);
...
function ss_playpause() {
  ...
  confixWrapper("functionCall", "dg('ss_playpause_link')", ["ss_playpause_link"], ['ss_playpause_link'], "ss_playpause", dg('ss_playpause_link')).innerHTML = confixWrapper("infix", "dg('ss_playpause_link').innerHTML = (ss_play) ? 'Pause it' : 'Play it'", [""], [], "ss_playpause", (ss_play) ? 'Pause it' : 'Play it');
  confixWrapper("functionCall", "dg('ss_playpause_link2')", ["ss_playpause_link2"], ['ss_playpause_link2'], "ss_playpause", dg('ss_playpause_link2')).innerHTML = confixWrapper("infix", "dg('ss_playpause_link2').innerHTML = dg('ss_playpause_link').innerHTML", [""], [], "ss_playpause", confixWrapper("functionCall", "dg('ss_playpause_link')", ["ss_playpause_link"], ['ss_playpause_link'], "ss_playpause", dg('ss_playpause_link')).innerHTML);
...
function src_smaller(x) {
  ...
  return confixWrapper("return", "return x.substr(0, x.length - 5) + \"4.jpg\";", [""], [], "src_smaller", confixWrapper("functionCall", "x.substr(0, x.length - 5)", ["0", "x.length - 5"], [0, x.length - 5], "src_smaller", x.substr(0, x.length - 5)) + "4.jpg");
}
function ss_toggleSmaller() {
  ss_smaller = confixWrapper("infix", "ss_smaller = !ss_smaller", [""], [], "ss_toggleSmaller", !ss_smaller);
  confixWrapper("functionCall", "dg('ss_smaller_link')", ["ss_smaller_link"], ['ss_smaller_link'], "ss_toggleSmaller", dg('ss_smaller_link')).innerHTML = confixWrapper("infix", "dg('ss_smaller_link').innerHTML = (ss_smaller) ? 'Larger Size' : 'Smaller Size'", [""], [], "ss_toggleSmaller", (ss_smaller) ? 'Larger Size' : 'Smaller Size');
  confixWrapper("functionCall", "dg('ss_photo')", ["ss_photo"], ['ss_photo'], "ss_toggleSmaller", dg('ss_photo')).src = confixWrapper("infix", "dg('ss_photo').src = ss_smaller ? src_smaller(dg('ss_photo').src) : ss_src[ss_cur]", [""], [], "ss_toggleSmaller", ss_smaller ? confixWrapper("functionCall", "src_smaller(dg('ss_photo').src)", ["dg(ss_photo).src"], [confixWrapper("functionCall", "dg('ss_photo')", ["ss_photo"], ['ss_photo'], "ss_toggleSmaller", dg('ss_photo')).src], "ss_toggleSmaller", src_smaller(confixWrapper("functionCall", "dg('ss_photo')", ["ss_photo"], ['ss_photo'], "ss_toggleSmaller", dg('ss_photo')).src)) : ss_src[ss_cur]);
}
function ss_run() {
  if (confixWrapper("condition", "(ss_play) && (ss_awaits <= 0)", [""], [], "ss_run", (ss_play) && (ss_awaits <= 0))) 
  {
    ss_awaits++;
    var lookup = confixWrapper("initvar", "var lookup = parseInt(dg('ss_refresh').value)", [""], [], "ss_run", parseInt(confixWrapper("functionCall", "dg('ss_refresh')", ["ss_refresh"], ['ss_refresh'], "ss_run", dg('ss_refresh')).value));
